...
class PreciseScrollingStep {
  constructor({ idx = -1, coverage = 0, focus = false, ratio = 0 }) {
    this.idx = idx
...
const PreciseScrolling = ({
  ...
  const evaluateChildrenPositions = () => {
    if (elementRef.current) {
      ...
      console.debug('[PreciseScrolling] evaluateChildrenPositions', '\n - numKids:', numKids)
  ...
  useScrollPosition(
    ({ prevPos, currPos }) => {
      const direction = prevPos.y < currPos.y ? DirectionDown : DirectionUp
      const activeSteps = []
      ...
      for (let i = 0; i < offsetTops.current.length; i += 1) {
        ...
        const isFocused =
          relativeOffsetTop < viewport.current.height / 2 &&
  ...
  return (
    <div className="PreciseScrolling" ref={elementRef}>
      {children}
    </div>
  )
}
...
// export default memo(PreciseScrolling, (prevProps, nextProps) => {
