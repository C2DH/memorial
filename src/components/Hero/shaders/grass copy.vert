...
void main() {
    ...
    float windForce = (noiseTexture.g - 0.35);
    ...
    float distCorrection = pow(length(instanceUv - vec2(0.5)), 2.0);

    scaledPosition.x *= 0.75 + randIDa * 0.75;
    scaledPosition.x *= 1.0 + 6.0 * distCorrection;
    ...
    bendPosition.x += flexFactor * windForce * 1.3;
    bendPosition.y -= pow(flexFactor, 2.0) * windForceNormal * 1.1;
    bendPosition.z += flexFactor * windForce * 0.3;
