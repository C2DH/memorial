...
const PeopleCard = ({ debug, alt, src, ...props }) => {
  ...
  */
  useLayoutEffect(()=>{//Changing card title value using timeout in order to let it fadeOut with old value and then change its value and fadeIn
    let t1
    let t2
    if(firstAnim){//Check if the card was animated for the first time, if not run first-time animation
    ...
    t1 = setTimeout(()=>{//timeout that changes card title when it faded out
      if(selectedPebble){
        setValue(selectedPebble)
      }
    }, 1350)
    t2 = setTimeout(()=>{//timeout that removes animation-name parameter from the card when the animation is over
      if(selectedPebble){
        setAnimate(false)
      }
    }, 2500)
  ...
  return (
    <div className={`PeopleCard ${animate ? 'cardAnimation' : ''} ${firstAnim ? 'cardAnimationIN' : ''}`} {...props}>
